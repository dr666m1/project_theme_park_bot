...
## 割り勘
### 出費の記録
`Y`に続けて半角数字を入力すると自分の出費を記録できます（`Y`はYenの頭文字です）。
...
## ペア分け
一行目に`C`、二行目以降にメンバーを入力するとランダムにペア分けされます（`C`はCombinationの頭文字です）。
...
### ペア分けの属性に性別以外を使いたい
性別だけでなく、たとえば単順に`A`や`B`など何を指定しても大丈夫です。

ただし、`A` `B` `C`のようにある属性で3種類以上の値が存在すると不平等が生じるかもしれません（未検証）。
